(function () {
  ...
  // GPSC titles in the supported corpus come in two recurring shapes:
  //   "GPSC 2026 Prelims – Advt 9994/2026-27 – English" / "...– Gujarati"
  //   "GPSC Class 1-2 - 2021 - 21 March - GS Paper 1 - English" / "...Gujarati"
  //   "GPSC Class 1-2 - 2025 - 20 April - General Studies - English" / "...Gujarati"
  // Year, advertisement number, sitting date, paper number, subject label,
  // and language are never matched or captured — the same
  // sitting/language-agnostic convention the title patterns above already
  // follow. The only substring common to every real GPSC title in the
  // corpus is "GPSC" as the title's first word (tested against `title`,
  // already stripped of its leading "#", like every other pattern here);
  // there is no second keyword ("Prelims" or otherwise) shared by all of
  // them, so none is required. An unrecognized title that merely contains
  // "GPSC" elsewhere (not as the first word) still falls through to `null`
  // (fail-closed).
  const GPSC_TITLE_PATTERN = /^GPSC\b/i;
  ...
  function detectExamType(rawMarkdown) {
    ...
    if (GPSC_TITLE_PATTERN.test(title)) {
      return "GPSC";
    }
  ...
  // --- GPSC Prelims (separate internal execution path — does not touch
  // parse()/parseUpsc()/parseNda()/parseGat()/parseCat() above) ---

  // GPSC's Directions/group markers are not written one consistent way
  // across papers. The 2026 sample's clean "(Directions for N to M) ..." is
  // only one of at least 8 further dialects found across the older "Class
  // 1-2" corpus (samples/md/, reconnaissance dated 2026-09-22), varying
  // independently in: singular/plural wording ("Direction"/"Directions" —
  // sometimes both forms appear in the SAME source file), whether the
  // label sits inside or outside the parentheses, whether "Questions"/
  // "Question No."/"Q.No."/"Q. No." is present, the range separator ("to",
  // a hyphen, or "and" for a 2-question range), optional bold-wrapping (of
  // the label only, or of the whole line including its trailing sentence),
  // and an optional leading "★" bullet. Real examples seen:
  //   (Directions for 10 to 11) Refer the information...
  //   **(Directions for Questions 156-160) :** Study the table...
  //   (Directions : Question No. 156 to 159) : The following table...
  //   (Direction : Question No. 152 and 153) In both the questions...
  //   Directions : (Question No. 182 to 187) : Study the following...
  //   ★ Directions (Q.No. 151 to 153) Find the missing element...
  //   Direction : (Q. No. 180 to 182): Two statements are given...
  // Gujarati translates the label to "નિર્દેશ"/"નિર્દેશો" (unlike the 2026
  // sample, where the English label itself is left untranslated) and can
  // place the translated label either before or after the declared range
  // within the same parenthesized group — e.g. "(પ્રશ્નો 156-160 માટે
  // નિર્દેશ) :" (range, then label) vs "(નિર્દેશ : પ્રશ્ન નં. 156 થી 159)"
  // (label, then range). None of this is inferred from meaning — a line
  // qualifies as a marker only if it structurally contains both a
  // Directions-shaped keyword and a two-number range, in either order, in
  // one of the separator shapes actually observed. No existing exam's
  // directions marker (UPSC/NDA/GAT/CAT) matches any of these shapes.
  //
  // A further dialect surfaced by the older 2017 "Class 1-2" General
  // Studies papers (samples/md2/, reconnaissance dated 2026-09-25): the
  // label itself is "Instructions"/"સૂચના" rather than "Directions"/
  // "નિર્દેશ" — a genuinely different keyword, not a spelling variant of
  // one already matched — and the declared range can appear with its two
  // numbers bolded individually, with the separator sitting outside both
  // bold spans (e.g. "પ્રશ્ન નં. **160** થી **162**"), or joined by an en
  // dash instead of a hyphen (e.g. "Q. No. **194 – 195**"). Real examples:
  //   **Instructions :** Read the following information and answer the
  //     questions **160 to 162** :
  //   **સૂચના :** નીચે આપેલ માહિતીનો અભ્યાસ કરી પ્રશ્ન નં. **160** થી
  //     **162** ના જવાબ આપો.
  //   **Instructions :** ... (Q. No. **194 – 195**) : Mark the correct
  //     answer.
  //
  // PARSER PHILOSOPHY (same as CAT's, see CAT_DIRECTIONS_MARKER_SOURCE):
  // the declared range IS the group boundary. It is read literally and
  // never inferred, extended, or cross-checked against content; a
  // transcription whose range is wrong is fixed in the Markdown, not here.
  const GPSC_DIRECTIONS_KEYWORD_SOURCE = "(?:\\bDirections?\\b|\\bInstructions?\\b|નિર્દેશો?|સૂચના)";
  const GPSC_DIRECTIONS_RANGE_SOURCE = "(\\d+)\\*{0,2}\\s*(?:to|and|-|–|—|થી|અને)\\s*\\*{0,2}(\\d+)";

  // A candidate marker line: contains the keyword AND the range shape
  // somewhere on the same line, in either order. Verified against the full
  // corpus (20 real GPSC files, both languages) to produce zero
  // false-positive matches on ordinary question/option/stem text — the
  // word "direction(s)" does appear on its own in ordinary prose (e.g.
  // "...give executive directions to state...") but never alongside a
  // two-number range on the same line anywhere in that corpus.
  const GPSC_DIRECTIONS_LINE_SOURCE = `^(?=.*${GPSC_DIRECTIONS_KEYWORD_SOURCE})(?=.*${GPSC_DIRECTIONS_RANGE_SOURCE}).+$`;

  // Extracts a marker line's own trailing instruction text, independent of
  // where the label/keyword sits relative to the range: takes whatever
  // follows the range's own closing parenthesis — skipping past label
  // words that sometimes repeat AFTER the range but BEFORE that paren
  // (e.g. Gujarati's "(પ્રશ્નો 156-160 માટે નિર્દેશ)": range, then the
  // label restated, then the paren closes) — then trims light surrounding
  // punctuation (":", "*", whitespace) left over from bold-wrapping or a
  // trailing colon, and any trailing "**"/"*" from a bold span that closes
  // at the end of the line rather than right after the range. Falls back
  // to trimming directly after the range if no closing parenthesis follows
  // it on the line (not observed in this corpus, but a safe degrade rather
  // than a throw).
  function extractGpscMarkerTrailingText(line, rangeEndIndex) {
    const after = line.slice(rangeEndIndex);
    const closeParenIndex = after.indexOf(")");
    const remainder = closeParenIndex === -1 ? after : after.slice(closeParenIndex + 1);
    return remainder.replace(/^[)\]:*\s]+/, "").replace(/\*{1,2}\s*$/, "").trim();
  }

  // GPSC's own option marker: parenthesized UPPERCASE letters, "(A)".."(D)".
  // Deliberately separate from UPSC_OPTION_MARKER_SOURCE (lowercase "(a)"),
  // which is left untouched; the letter is already the target key, so no
  // case-mapping table is needed.
  const GPSC_OPTION_MARKER_SOURCE = "^\\(([A-D])\\)\\s*";

  function findGpscDirectionsMarkers(rawMarkdown) {
    const linePattern = new RegExp(GPSC_DIRECTIONS_LINE_SOURCE, "gmi");
    const rangePattern = new RegExp(GPSC_DIRECTIONS_RANGE_SOURCE, "i");
    const markers = [];
    let lineMatch;

    while ((lineMatch = linePattern.exec(rawMarkdown)) !== null) {
      const line = lineMatch[0];
      const rangeMatch = rangePattern.exec(line);
      ...
      markers.push({
        ...
        contentStart: lineMatch.index + line.length,
        instructionText: extractGpscMarkerTrailingText(line, rangeMatch.index + rangeMatch[0].length),
        declaredStart: Number(rangeMatch[1]),
        declaredEnd: Number(rangeMatch[2]),
      });
    }
  ...
  // Each marker governs exactly its declared [declaredStart, declaredEnd]
  // question numbers, clipped only by the next marker (a safety clamp
  // against overlapping markers, not an inference step). Instruction text
  // is the marker line's own trailing text followed by the gap text between
  // the marker line and the first governed question, in document order,
  // skipping whichever side is empty and never duplicating — the same
  // source-preservation rule computeCatAssignments already applies.
  // Produces {question, passage} tuples in the exact shape
  // passageTextByQuestionIndex/questionGroupsByQuestionIndex expect.
  function computeGpscAssignments(rawMarkdown, questionBoundaries) {
    const directionsMarkers = findGpscDirectionsMarkers(rawMarkdown);
    const assignments = [];
  ...
  // A question's own raw block trails into the NEXT group's Directions line
  // and shared content (they sit after that question's answer line, before
  // the next question marker), which belongs to the following group, never
  // to this question — cut it off before the answer/options are read.
  // Reuses the same dialect-tolerant line pattern findGpscDirectionsMarkers
  // is built from, so a truncation boundary is recognized under exactly the
  // same rules a group-boundary marker is.
  const GPSC_DIRECTIONS_LINE_PATTERN = new RegExp(GPSC_DIRECTIONS_LINE_SOURCE, "mi");

  function truncateBeforeNextGpscDirectionsMarker(content) {
    const match = GPSC_DIRECTIONS_LINE_PATTERN.exec(content);
    return match ? content.slice(0, match.index).trimEnd() : content;
  }

  // Same composition as splitUpscOptions, with GPSC_OPTION_MARKER_SOURCE.
  function splitGpscOptions(beforeAnswer) {
    const pattern = new RegExp(GPSC_OPTION_MARKER_SOURCE, "gm");
    const matches = [];
    let match;
    ...
    matches.forEach((optionMatch, i) => {
      const optionEnd = i + 1 < matches.length ? matches[i + 1].index : beforeAnswer.length;
      options[optionMatch.letter] = beforeAnswer.slice(optionMatch.contentStart, optionEnd).trimEnd();
    });

    return { questionMarkdown, options };
  }

  // Answers are inline "**Ans.** (X)" — the same line shape UPSC's own
  // splitAtUpscAnswerMarker/resolveUpscCorrectAnswer already handle, reused
  // unchanged (the resolver already accepts uppercase A-D). Any other
  // payload on the Ans. line resolves to null, exactly as for a paper with
  // no key — no GPSC-specific answer handling exists.
  function parseGpscBlock(block, stemByQuestionIndex, groupByQuestionIndex) {
    const ownContent = truncateBeforeNextGpscDirectionsMarker(block.content);
    const { beforeAnswer, answerText } = splitAtUpscAnswerMarker(ownContent);
    const { questionMarkdown, options } = splitGpscOptions(beforeAnswer);
    const { hasImage, imageMarkdown } = detectImage(block.rawBlock);
    const correctAnswer = resolveUpscCorrectAnswer(answerText);
    const stemText = stemByQuestionIndex.get(block.index) || null;
    // Legacy/rollback field, same convention as UPSC/NDA/GAT/CAT: shared
    // instruction folded in, though the live Question Group runtime pastes
    // questionOnlyMarkdown for a grouped question.
    const fullQuestionMarkdown = stemText ? `${stemText}\n\n${questionMarkdown}` : questionMarkdown;
  ...
  function parseGpsc(rawMarkdown) {
    if (typeof rawMarkdown !== "string") {
      return [];
    }

    const questionBoundaries = findQuestionBoundaries(rawMarkdown);
    const assignments = computeGpscAssignments(rawMarkdown, questionBoundaries);
    const stemByQuestionIndex = passageTextByQuestionIndex(assignments);
    const groupByQuestionIndex = questionGroupsByQuestionIndex(assignments);

    return splitIntoBlocks(rawMarkdown).map((block) => parseGpscBlock(block, stemByQuestionIndex, groupByQuestionIndex));
  }
  ...
  function parseDocument(rawMarkdown) {
    ...
    if (examType === "GPSC") {
      return { examType, questions: parseGpsc(rawMarkdown) };
    }

    return { examType: null, questions: [] };
  }

  window.ExamUploadAssistantParser = { parse, parseUpsc, parseNda, parseGat, parseCat, parseGpsc, detectExamType, parseDocument };
})();
