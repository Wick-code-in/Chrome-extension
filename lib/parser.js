(function () {
  ...
  // after the colon.
  //
  // CRITICAL: the declared count/range captured by groups 1-3 is NEVER
  // trusted for group-boundary computation (see computeCatAssignments) —
  // only used, if at all, as a fallback instruction text when the gap to
  // the first governed question is empty. The approved CAT sample proves
  // this declared range can undercount the real structural range: the
  // "questions 17-18" header actually governs Q17-20, and the
  // "questions 43-45" header actually governs Q43-46. This is why CAT
  // follows GAT's structural approach (every question boundary up to the
  // next marker/section) rather than NDA Mathematics's declared-count
  // approach (`.slice(0, itemCount)`). This is also why the "-" vs "&"
  // range separator is never captured as its own group — only used to
  // recognize the marker at all, never to interpret the declared range.
  ...
  // [2 - 5]**" (CAT_2024_Slot_3.md, bold-wrapped). Corpus-verified (see the
  // CAT-wide reconnaissance): 13 occurrences across two papers, every single
  // declared [N - M] range exactly equal to its real structural group — a
  // stronger guarantee than DIRECTIONS's free-text range ever offers, but
  // per the same CRITICAL note below, still never trusted for boundary
  // computation; only DIRECTIONS/Instructions markers together and Section
  // markers ever decide a group's actual extent (see computeCatAssignments).
  // Semantically this is the exact same construct as DIRECTIONS — it just
  // uses a bracketed numeric range instead of DIRECTIONS's colon-terminated
  // free text, and always carries its own instruction/passage prose on
  // separate following lines rather than trailing the marker itself (unlike
  // DIRECTIONS, whose group 4 sometimes carries real trailing text on the
  // marker's own line). Optional "**" wrapping is stripped by \*{0,2} on
  // both ends, matching plain and bold forms with one pattern. Groups 1/2
  // capture the declared start/end — captured for symmetry with
  // CAT_DIRECTIONS_MARKER_SOURCE and never trusted for boundaries, same as
  // there. No same-line trailing-text group exists here (nothing has ever
  // been observed after the closing bracket/asterisks in this corpus) — a
  // hypothetical "Instructions [N-M] [FIGURE]" form is intentionally left
  // unsupported since it has no corpus evidence; see findCatDirectionsMarkers.
  ...
  // and merges them into one list, sorted back into document order —
  // computeCatAssignments treats the merged list as a single sequence of
  // group-boundary markers. An Instructions match never has same-line
  // trailing text (see CAT_INSTRUCTIONS_MARKER_SOURCE's comment), so its
  // instructionText is always "" — computeCatAssignments's existing
  // empty-gap fallback already handles that correctly with no changes
  // needed there.
  //
  // Each marker also carries `kind` ("directions" | "instructions") and,
  // for Instructions markers, `declaredStart`/`declaredEnd` — the two
  // numbers captured from its bracketed range. computeCatAssignments uses
  // `kind` to decide whether the declared range is trusted as an
  // authoritative boundary (Instructions only) or purely structural
  // (DIRECTIONS, unchanged). See computeCatAssignments's own comment for
  // why the two dialects are deliberately treated differently here despite
  // sharing this same marker-finding function.
  ...
  // Structural group-boundary computation, mirroring GAT's own
  // computeGatAssignments rather than NDA Mathematics's declared-count
  // approach (see CAT_DIRECTIONS_MARKER_SOURCE's comment for why). For each
  // DIRECTIONS marker, the governed range is every question boundary up to
  // the earliest of: the next DIRECTIONS/Instructions marker, the next
  // Section marker, or EOF — never a declared-count `.slice()` cap. The
  // same empty-gap vs. non-empty-gap instruction split GAT introduced is
  // reused unchanged: if real content sits between the marker and the
  // first governed question, that content becomes the shared instruction;
  // otherwise the marker's own captured trailing text becomes the
  // instruction instead. Produces {question, passage} tuples in the exact
  // shape questionGroupsByQuestionIndex (defined above, for UPSC) already
  // expects — reused here completely unchanged, the same reuse NDA
  // Mathematics and GAT already established.
  //
  // EXCEPTION — Instructions markers only: corpus-wide reconnaissance
  // across every CAT sample found 13 "Instructions [N-M]" markers, and
  // every single one's declared bracketed range exactly equalled its real
  // semantic group — a stronger guarantee than DIRECTIONS's free-text
  // range ever offers (DIRECTIONS is proven, twice, to undercount). Absent
  // a per-question marker for every standalone question, the structural
  // window alone cannot distinguish "the group genuinely continues" from
  // "an unrelated standalone question happens to sit before the next
  // marker" — this is exactly what incorrectly swept CAT_2024_Slot_1.md's
  // Q5/Q10-12/Q17-20 and CAT_2024_Slot_3.md's Q6-7/Q12/Q17-20 into the
  // preceding Instructions group. So, for Instructions markers only (never
  // DIRECTIONS), the declared [declaredStart, declaredEnd] range is
  // additionally applied as a hard filter on top of the structural window
  // below — trusted as authoritative, the opposite of DIRECTIONS's
  // philosophy, deliberately, because the corpus evidence for the two
  // dialects points in opposite directions. This does trade away
  // robustness against a mis-transcribed Instructions range (an undercount
  // there would now silently drop a real member, the same failure mode
  // DIRECTIONS's structural approach exists to avoid) — accepted because
  // zero such cases have been observed across the corpus, unlike DIRECTIONS.
  function computeCatAssignments(rawMarkdown, questionBoundaries) {
    ...
    directionsMarkers.forEach((marker, i) => {
      ...
      const structuralQuestions = questionBoundaries.filter(
        (boundary) => boundary.index > marker.index && boundary.index < blockEnd
      );

      // Instructions: additionally cap to the declared range — see the
      // EXCEPTION note above. DIRECTIONS: unchanged, purely structural.
      const governedQuestions =
        marker.kind === "instructions"
          ? structuralQuestions.filter(
              (boundary) => boundary.questionNumber >= marker.declaredStart && boundary.questionNumber <= marker.declaredEnd
            )
          : structuralQuestions;
      ...
      const gapText = rawMarkdown.slice(marker.contentStart, governedQuestions[0].index).trim();
      // gapText normally wins outright over the marker's own same-line
      // trailing text (real passage content beats generic boilerplate like
      // "The passage below is accompanied by four questions..."). But a
      // literal "[FIGURE]" on the marker line (e.g. CAT_2024_Slot_2.md's
      // "DIRECTIONS for questions 30-33: [FIGURE]") is not generated
      // boilerplate — it's a real, load-bearing operator cue placed there
      // during transcription to flag a chart/image for manual handling in
      // Docxsity — so it must never be silently dropped just because real
      // gap content also exists. This check is deliberately narrow (only
      // "[FIGURE]", not "always keep marker text") to avoid reintroducing
      // boilerplate into the dozens of already-correct groups whose marker
      // text is just descriptive prose. Order is preserved: the marker's
      // own line always precedes the gap in the source document. Applies
      // uniformly to both marker dialects (DIRECTIONS and Instructions) —
      // nothing here is keyed to either literal marker keyword.
      const instructionText =
        gapText.length > 0
          ? marker.instructionText.includes("[FIGURE]")
            ? `${marker.instructionText}\n\n${gapText}`
            : gapText
          : marker.instructionText;
      const passage = { id: i, text: instructionText };
