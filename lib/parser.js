(function () {
  ...
  // A common-instruction line governing the next N questions, e.g. "Consider
  // the following for the three (03) items that follow :" (English) or
  // "आने वाले तीन (03) प्रश्नांशों के लिए निम्नलिखित पर विचार कीजिए :"
  // (Hindi). Structurally the reverse of UPSC's DIRECTIONS_MARKER_SOURCE —
  // NDA spells the count out as a word and puts the digits in parens
  // ("three (03)"), where UPSC puts a bare digit before the parenthesized
  // word ("3 (three)") — and NDA has no separate "Passage – N" label line at
  // all; the shared stem text simply starts right after this line. Matched
  // structurally (any colon-terminated line containing a parenthesized
  // number) rather than by hardcoding either language's wording, consistent
  // with the project's existing UPSC marker philosophy. Verified against
  // both NDA sample files: exactly 27 matches per language, zero false
  // positives elsewhere in either document.
  ...
  window.ExamUploadAssistantParser = { parse, parseUpsc, parseNda, detectExamType, parseDocument };
})();
