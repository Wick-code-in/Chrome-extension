(function () {
  ...
  // "DIRECTIONS for the question 1: ..." / "DIRECTIONS for questions 2-3:
  // ...". Structurally similar to UPSC's/NDA's own directions markers, but
  // CAT's own dialect: literal "DIRECTIONS" in caps (not "Directions"), and
  // two distinct shapes for a single governed question vs. a range. Group 1
  // captures a lone question number, groups 2/3 capture a range's start/end,
  // group 4 captures the trailing instruction text after the colon.
  //
  ...
  // approach (`.slice(0, itemCount)`).
  const CAT_DIRECTIONS_MARKER_SOURCE = "^DIRECTIONS for (?:the question (\\d+)|questions (\\d+)-(\\d+)):\\s*(.*)$";

  // CAT's own bare-digit MCQ option marker — "1. ...", "2. ...", "3. ...",
  // "4. ...", plain numbered-list lines, structurally distinct from JEE's
  // parenthesized "(1)" and UPSC's/NDA's/GAT's lettered "(a)". Only ever
  // invoked (see splitCatOptions below) for a question already determined
  // to be MCQ — never run against TITA content, since several genuine TITA
  // questions (Q4, Q5, Q15, Q16 in the approved sample) embed their own
  // bare-numbered list as part of the question body (a jumbled-sentence /
  // sequencing puzzle), which is structurally indistinguishable from a real
  // options list by this regex alone. Type is decided from the explicit
  // [TITA] marker (see CAT_TITA_TAG_PATTERN) before this is ever reached.
  const CAT_OPTION_MARKER_SOURCE = "^([1-4])\\.\\s*";
  ...
  function findCatDirectionsMarkers(rawMarkdown) {
    const pattern = new RegExp(CAT_DIRECTIONS_MARKER_SOURCE, "gm");
    const markers = [];
    let match;
  ...
  // the earliest of: the next DIRECTIONS marker, the next Section marker,
  // or EOF — never a declared-count `.slice()` cap. The same empty-gap vs.
  // non-empty-gap instruction split GAT introduced is reused unchanged: if
  // real content sits between the marker and the first governed question,
  // that content becomes the shared instruction; otherwise the marker's own
  // captured trailing text becomes the instruction instead. Produces
  // {question, passage} tuples in the exact shape questionGroupsByQuestionIndex
  // (defined above, for UPSC) already expects — reused here completely
  // unchanged, the same reuse NDA Mathematics and GAT already established.
  ...
  // A question's own raw block can trail into the next DIRECTIONS line's
  // preamble or the next Section heading — the same reason every other exam
  // in this file needs its own truncator. Unions both CAT marker shapes in
  // one pattern, the same way GAT's own truncateBeforeNextGatBoundary unions
  // its Directions and Part markers.
  const CAT_BOUNDARY_PATTERN = new RegExp(`${CAT_DIRECTIONS_MARKER_SOURCE}|${CAT_SECTION_MARKER_SOURCE}`, "m");
  ...
  // CAT_OPTION_MARKER_SOURCE's bare-digit shape and DIGIT_TO_LETTER's
  // existing digit-to-letter map (both already defined above, for JEE)
  // reused unchanged.
  ...
  function parseCatBlock(block, sectionMarkers, groupByQuestionIndex) {
    ...
    if (isTita) {
      ...
      correctAnswer = answerText && DIGIT_TO_LETTER[answerText] ? DIGIT_TO_LETTER[answerText] : null;
    }
