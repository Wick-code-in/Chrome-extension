(function () {
  ...
  // GPSC titles look like "GPSC 2026 Prelims – Advt 9994/2026-27 – English"
  // / "...– Gujarati". Year, advertisement number, and language are never
  // matched or captured, the same sitting/language-agnostic convention the
  // title patterns above already follow. Anchored on "GPSC" as the title's
  // first word (tested against `title`, already stripped of its leading
  // "#", like every other pattern here) and separately required to contain
  // "Prelims" (checked at the call site in detectExamType) — a GPSC Mains
  // title, or any other GPSC document, therefore falls through to `null`
  // (fail-closed) rather than being routed to a parser built only for the
  // Prelims format.
  ...
  function detectExamType(rawMarkdown) {
    ...
    if (GPSC_TITLE_PATTERN.test(title) && /Prelims/i.test(title)) {
      return "GPSC";
    }
