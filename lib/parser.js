(function () {
  ...
  // "(Directions for 10 to 11) Refer the information given below and answer
  // Q-10 and Q-11 based on that:" — a parenthesized label declaring an
  // explicit question range ("N to M"), followed on the SAME line by real
  // instruction text (group 3), with further shared content on the lines
  // after it. The "(Directions for N to M)" label itself is structural and
  // is never kept as instruction content; only the text after it is. The
  // literal English label is identical in the English and Gujarati samples
  // (only the trailing sentence is translated), so one pattern serves both
  // languages with no per-language branch. No existing exam's directions
  // marker (UPSC/NDA/GAT/CAT) matches this shape — verified against both
  // samples' five markers — hence its own pattern rather than a change to
  // any of theirs.
  //
  ...
  const GPSC_DIRECTIONS_MARKER_SOURCE = "^\\(Directions for (\\d+) to (\\d+)\\)[ \\t]*(.*)$";
  ...
  function findGpscDirectionsMarkers(rawMarkdown) {
    const pattern = new RegExp(GPSC_DIRECTIONS_MARKER_SOURCE, "gm");
    const markers = [];
    let match;
  ...
  const GPSC_DIRECTIONS_PATTERN = new RegExp(GPSC_DIRECTIONS_MARKER_SOURCE, "m");

  function truncateBeforeNextGpscDirectionsMarker(content) {
    const match = GPSC_DIRECTIONS_PATTERN.exec(content);
    return match ? content.slice(0, match.index).trimEnd() : content;
  }
