(function () {
  ...
  //                          sibling <label>'s exact text (no for/id link
  //                          exists between them). Returns the first match
  //                          of `find` within the label's parent — defaults
  //                          to "input, select, textarea" for plain fields,
  //                          but can be any CSS string (e.g. to find a
  //                          specific toolbar button within a labeled
  //                          rich-text editor, scoped away from other
  //                          identical-looking editors elsewhere on the page).
  ...
  function findByLabelText(labelText, root, searchSelector = "input, select, textarea") {
    ...
    return label.parentElement.querySelector(searchSelector);
  }
