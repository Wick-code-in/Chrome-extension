(function () {
  ...
  function selectDropdown(selector, value, { root = document } = {}) {
    const element = findElement(selector, root);
    console.log("[Exam Upload Assistant] selectDropdown: element found:", element);
    ...
    console.log("[Exam Upload Assistant] selectDropdown: value before selection:", element.value);
    console.log("[Exam Upload Assistant] selectDropdown: target value:", value);

    getNativeValueSetter(element).call(element, value);
    console.log("[Exam Upload Assistant] selectDropdown: value immediately after native setter:", element.value);

    element.dispatchEvent(new Event("input", { bubbles: true }));
    element.dispatchEvent(new Event("change", { bubbles: true }));
    console.log("[Exam Upload Assistant] selectDropdown: value after dispatching events:", element.value);
    ...
    if (element.value !== value) {
      ...
      console.log("[Exam Upload Assistant] selectDropdown: returning FAILURE:", failureResult);
      return failureResult;
    }

    const successResult = { success: true, message: `Selected dropdown value "${value}": ${describeSelector(selector)}`, retryable: false };
    console.log("[Exam Upload Assistant] selectDropdown: returning SUCCESS:", successResult);
