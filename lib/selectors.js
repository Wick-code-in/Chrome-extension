(function () {
  ...
  //     text, then returns the first match of `find` within the label's
  //     parent (defaults to "input, select, textarea") — used for fields
  //     where a sibling <label> carries the only stable identifier (no
  //     `for`/`id` link exists), optionally scoping a further CSS search
  //     (e.g. a specific toolbar button) to that one labeled field, or
  ...
  const SELECTORS = {
    ...
    pasteOptions: {
      markdownButton: (letter) => `[data-testid="option-${letter.toLowerCase()}-markdown-button"]`,
    },
    markCorrect: {
