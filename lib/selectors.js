(function () {
  ...
  const SELECTORS = {
    prepareForm: {
      ...
      //
      // KNOWN LIMITATION (open, not fixed — see CHANGELOG.md): on the
      // specific transition of a standalone question immediately following
      // a Question Group, this dropdown may occasionally fail to select
      // MCQ (Marks/Penalty/Question Text are unaffected). Rare and
      // transition-specific; multiple live-diagnosis attempts did not
      // produce a confirmed fix, and the investigation was deliberately
      // shelved rather than risk destabilizing the working Question Group
      // flow. Manual workaround: select MCQ manually on the site and
      // continue with Execute Step / Pass Step as normal.
