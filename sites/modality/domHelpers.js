(function () {
  ...
  async function pasteMarkdown(selectors, markdownText, options = {}) {
    ...
    // `options.root`, when given, scopes only the trigger button lookup
    // (e.g. to one Sub Question card among several with an identically
    // labeled field) — the "Paste Markdown Data" modal itself is a single,
    // page-level dialog regardless of which field opened it, so every
    // lookup from here on deliberately searches from `modalOptions`
    // (`options` with `root` stripped back out to its own document
    // default) rather than the caller's scoped root. For every existing
    // caller (which never passes root at all) this is a no-op: `options`
    // was always `{}` already, so `modalOptions` is identical to it.
