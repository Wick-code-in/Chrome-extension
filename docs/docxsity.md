...
- **SAVE**: only the group's **last** member (`isLastInGroup`) ever reaches `SAVE` — a Question Group is saved once, as a whole, not per sub-question. `determineNextState()` special-cases this: from `GENERATE_AI`, if `question.group && !question.group.isLastInGroup`, the next state is `NEXT_QUESTION` directly (skipping `ADD_TAGS`/`SAVE` entirely) — this branch is nested inside Modality's `isUpscExamType()` check, since UPSC/NDA are the only exam types with group data in practice today (see [nda-mathematics.md](nda-mathematics.md) §4 for the NDA-specific extension to that predicate). **This is Modality's behavior only** — Docxsity's own equivalent check (below) is keyed differently, since unlike UPSC/NDA, most Docxsity exam types have a real per-question subject to tag.
...
**Composes with Question Groups automatically, with zero group-aware code added.** All four checks read only `Settings` (plus, where relevant, `question.type`/`question.correctAnswer`/`question.subject`) — none inspect `question.group`. Since `MARK_CORRECT`/`GENERATE_AI`/`ADD_TAGS` already run once per group member regardless of position (only `SAVE` is skipped for non-last members, by the pre-existing group logic above), a toggle-driven skip applies identically to a standalone question, a passage group, an instruction-only group, or a large multi-member group — nothing new was needed to make this true.
...
**Question Groups:** unlike `SAVE`, `ADD_TAGS` runs for **every** group member, not just the last — see [Question Groups](#question-groups) above (Phase 3C) for why the group-skip gate sits after `ADD_TAGS` rather than after `GENERATE_AI`. Each member is tagged with its own `question.subject`, scoped to its own Sub Question card via `resolveCurrentRoot(question)` — the same per-member root every earlier state in the group already uses.
...
Not a state — an operator control (`jumpToQuestion(rawInput)`) that validates the input as a positive integer, then resolves it to a question by searching the loaded question list for the one whose `questionNumber` matches — never by treating the number as an array index (`questionNumber - 1`). `questionNumber` identifies a question; array index identifies where that question currently sits in the loaded list. Sequential source numbering makes the two numerically coincide, but Jump must not rely on that coincidence when resolving a question by number — `Session`'s own normal sequential navigation can keep using array indexes as-is, since it's navigating by position, not by an operator-supplied number. See [decisions.md](decisions.md) for the full rationale, including the real sparse-numbered paper that exposed the old array-arithmetic assumption as incorrect. A number with no matching question (including one that falls in a gap in the source numbering) fails with a clear "does not exist" message rather than silently resolving to an adjacent question.

Once resolved, `Session`'s current question index is set to the matched question's actual array position and state resets to `PREPARE_FORM`. Docxsity's version has the same group-redirect logic as Modality's (see [Question Groups](#question-groups)): jumping to any non-first group member redirects to that group's first question instead, since a Question Group can only be created sequentially starting from its first sub-question — this part is unchanged. Edge cases (all independently confirmed): jump to a group's first member → direct jump; jump to a middle or last member → redirect to first; a standalone question immediately before or after a group → unaffected, direct jump either way.
