...
**Re-confirmed by the NDA Mathematics addition.** NDA Mathematics parses 27 shared-instruction blocks (governing 59 of its 120 questions) into real, non-null `group` metadata in the same shape UPSC Paper II uses — but on Docxsity this changes nothing: `runPrepareForm()`/`runPasteQuestion()` here never read `question.group` or `question.questionOnlyMarkdown` at all, only `question.questionMarkdown` (which always carries the shared instruction folded in, for every exam type, grouped or not). Live-verified: an NDA question governed by a shared instruction uploads through Docxsity's existing standalone Add Question flow exactly like any other question, with the instruction text appearing inline at the top of the pasted Question field rather than through a Question Group UI. This is the flatten-and-duplicate behavior already described above for every other exam type, not new behavior introduced for NDA.
