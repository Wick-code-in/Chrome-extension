...
## Slideshow: TinyMCE trigger-button readiness race

**Symptom:** once Slideshow's zero-delay `executeStep()` loop (see [slideshow.md](slideshow.md)) was driving Docxsity end to end, `PASTE_QUESTION`/`PASTE_OPTIONS`/Question-Group-instruction pastes intermittently failed with element-not-found errors on the "Paste Raw Markdown" trigger button — a failure never observed during manual, one-click-at-a-time testing of the exact same states.

**Investigation:** live reconnaissance against a disposable Docxsity question bank, driving the real `pasteMarkdown()` composition directly with zero artificial delay between the triggering click and the next action (deliberately reproducing what Slideshow's tight loop does, unlike every prior manual test which always had an accidental human-speed gap). Measured, with `performance.now()`, the time between a freshly-created field's own container becoming queryable and that same field's "Paste Raw Markdown" toolbar button becoming queryable: **~34–55ms** for option cards (three samples: 53.7ms, 34.8ms, 33.8ms), **~53ms** for the Question Text field on a freshly-opened Add Question modal. A parallel check of plain Angular controls in the same containers (Question Type `ng-select`, Marks input, "Mark as Correct" button) showed **zero** measured lag, isolating the delay to TinyMCE's own per-field toolbar initialization specifically.

**Root cause:** a freshly-created TinyMCE-backed field's container node exists in the DOM measurably before that field's own toolbar (including its "Paste Raw Markdown" button) finishes initializing — a race invisible to manual use only because a human's own reaction time between clicks always exceeds the ~34–55ms gap by a wide margin.

**Fix:** `sites/docxsity/domHelpers.js`, inside `pasteMarkdown()` — added `await waitForElement(triggerButton, options)` immediately before the pre-existing `clickElement(triggerButton, options)`, reusing the existing primitive and existing selector/timeout, no new mechanism. Because every rich-text field on the site (Question Text, every Option, a Question Group's Instruction/Title, any Sub Question card's own fields) already goes through this one shared function, the single change covers all of them.

**Lesson:** an automation loop with no delay between steps can surface real, previously-invisible DOM-initialization races that manual, human-paced testing structurally cannot — because human reaction time between clicks incidentally functions as an ad-hoc wait. When adding any future automated (non-human-paced) execution path, budget time for a dedicated zero-delay live reconnaissance pass rather than assuming "it passed manual testing" implies it's race-free at automation speed. See [slideshow.md](slideshow.md) §10–§13 for the full narrative, including why a generic fixed delay was rejected in favor of this targeted wait.

---

## Slideshow: post-Save "Add Question" button readiness race

**Symptom:** during the same Slideshow zero-delay reconnaissance above, standalone questions occasionally failed immediately after a successful Save, with the next `PREPARE_FORM` unable to find the top-level "Add Question" button — again, never observed manually.

**Investigation:** a passive `MutationObserver` recorded every relevant DOM change after a Save click, with no click of "Add Question" attempted until the recording was analyzed after the fact (to avoid the observation itself perturbing the timing). One representative trial: Save clicked at t=0; "Add Question" button count dropped to 0 at t≈393ms — **while the Add Question modal was still visibly open**; the modal itself was confirmed gone (existing `waitForDisappear` signal) at t≈720ms; the "Add Question" button reappeared, in the same DOM mutation as the "Questions List" container, at t≈758ms. A second independent trial measured the modal-close-to-button-reappear gap directly via `waitForElement()` timing at **~43ms** (first trial's equivalent gap: ~38ms). Both trials reproduced the original failure when the click was attempted at the exact instant `waitForDisappear(addQuestionModal)` resolved.

**Root cause:** Docxsity tears down and recreates the "Add Question button + Questions List" view as one page-level transition after Save, running on its own timeline **not synchronized with the modal's own close signal** — the modal disappearing is necessary but not sufficient evidence that the "Add Question" button is ready; it lags by tens of milliseconds.

**Fix:** `sites/docxsity/stateMachine.js`, inside `ensureQuestionFormReady()` — added `await DomHelpers.waitForElement(selectors.addQuestionButton)` before the standalone-question click, and the identical `await DomHelpers.waitForElement(groupSelectors.addQuestionGroupButton)` before the Question-Group-entry click. Both reuse the existing primitive and existing selectors; no new selector, no timeout change, no retry logic. Verified fixed via 5 consecutive standalone questions run back-to-back with zero artificial delay (individual cycle times: 5876ms, 5485ms, 5646ms, 4083ms, 4188ms — all 4 `SAVE → PREPARE_FORM` boundary crossings succeeded), and separately via a Question-Group entry immediately following that run.

**Lesson:** a completion signal for one UI element (a modal's disappearance) is not automatically a valid readiness signal for a *different* element (a button elsewhere on the page), even when the two are conceptually part of "the same transition" — confirm each element's own readiness independently rather than assuming one implies the other. See [slideshow.md](slideshow.md) §12–§13.
